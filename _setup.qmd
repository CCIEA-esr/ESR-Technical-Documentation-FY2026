...
#  df = read.csv('CCIEA_metadata.csv')
  df = read.csv('https://raw.githubusercontent.com/CCIEA-esr/CCIEA-uploader/refs/heads/master/data/CCIEA_metadata.csv')
...
print_meta <- function(meta,i){
    ...
    cat("*")
    get_github_raw_file(meta$cciea_filename[i],meta$PI_filename[i],meta$no_download[i])
}

get_github_raw_file <- function(cciea_filename,PI_filename,nodnld){
   base="https://raw.githubusercontent.com/CCIEA-esr/CCIEA-uploader/refs/heads/master/data/timeseries_data/";
   cciea_filename_url=paste0(base,cciea_filename,sep="")
   PI_filename_url=paste0(base,PI_filename,sep="")
   if(is.na(nodnld)){
    if(url_check(cciea_filename_url)){
      cat("\n**Download data file: ** [GitHub raw csv file](",cciea_filename_url,")","*(right-click -> save page as)*\n",sep="")
    }
     else if (url_check(PI_filename_url)){
      cat("\n**Download data file: ** [GitHub raw csv file](",PI_filename_url,")\n","*(right-click -> save page as)*\n",sep="")
     }
     else{
     ...
     cat("\n**Private dataset, no download available **","\n",sep="")
   }
}
...
get_erddap_link <-function(erddap_id){
  s=""
  eid <- erddap_id
  eid <- eid[!str_ends(eid, "_ND")]
  if(length(eid)>1)s="s"
#  cat("**Indicator Download",s," **\n\n",sep="")
  cat("**ERDDAP&trade; link",s,": **\n",sep="")
  for(i in 1:length(eid)){
    cat("<https://oceanview.pfeg.noaa.gov/erddap/tabledap/",eid[i],".html> \n",sep="")
    }
  cat("")
  }
